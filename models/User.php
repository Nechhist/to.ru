...
class User extends ActiveRecord implements IdentityInterface
{
    ...
    {
        if($id==0) return '<i>Гость</i>';

        $user = User::findOne(['id' => $id, 'status' => self::STATUS_ACTIVE]);
        if($user==null) return '<i>==не_найден==</i>';
        if($a==0) return '<strong>'.HTML::encode($user['username']).'</strong>';
        if($a==1) return '<a href="'.Yii::$app->request->baseUrl.'/user/view/'.$user['id'].'">'.HTML::encode($user['username']).'</a>';
